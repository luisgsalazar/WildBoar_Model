...
# foreach (Scen = c(1:2), .export = c('HC2','HC3'), .packages = c("dplyr","flux"))  %dopar% {
#   path = paste("./Outputs/Scenario",Scen, sep = '_')
#   if (!dir.exists(path))  dir.create(path)
#   
#   WBModel(MaxIterations = 100, 
...
for (Scen in c(1:4)) {path = paste("./Outputs/Scenario",Scen, sep = '_')
if  (!dir.exists(path))  dir.create(path)


if(Scen %in%c(3:4)){WBModel(MaxIterations = 100, 
                            ProbHarvest   = Scenarios[Scenarios$Sc == Scen, "ProbHarvest"],
                            ProbHarvestAM = Scenarios[Scenarios$Sc == Scen, "ProbHarvestAM"],
                            ProbMovHunt   = Scenarios[Scenarios$Sc == Scen, "ProbMovHunt"],
                            runID = paste0(path, "/", Scen),
